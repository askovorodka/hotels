...
class Version20180816122339 extends AbstractMigration
{
        ...
        $this->addSql('
            create or replace view hotel_amenities_with_price as
                select `h`.`id`                  AS `id`,
                       ...
                       `h`.`price`               AS `price`,
                       `a`.`id`                  AS `amenity_id`
                from ((`hotel_with_min_price` `h` join `hotel_amenities` `ha` on ((`ha`.`hotel_id` =
                                                                                                   `h`.`id`))) join `amenity` `a` on ((
                  `ha`.`amenity_id` = `a`.`id`)))
        ');
    }
    ...
    {
        $this->abortIf($this->connection->getDatabasePlatform()->getName() !== 'mysql', 'Migration can only be executed safely on \'mysql\'.');
        $this->addSql('DROP VIEW hotel_amenities_with_price');
    }
}
