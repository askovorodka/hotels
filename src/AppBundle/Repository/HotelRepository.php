...
class HotelRepository extends EntityRepository
{
    ...
    /**
     * Возвращает запрос на получение всех locality отсортированных по алфавиту
     *
     * @param string $alias
     * @return QueryBuilder
     */
    public function getLocalitiesQuery($alias = 'h'): QueryBuilder
    {
        $queryBuilder = $this->createQueryBuilder($alias);
        $queryBuilder
            ->select("$alias.locality")
            ->distinct()
            ->andWhere($queryBuilder->expr()->isNotNull("$alias.locality"))
            ->andWhere("$alias.locality <> ''")
            ->addOrderBy("$alias.locality");
     ...
     * @return mixed
     */
    public function getCountActiveHotels()
    {
        /** @var EntityManager $em */
        $em = $this->getEntityManager();
